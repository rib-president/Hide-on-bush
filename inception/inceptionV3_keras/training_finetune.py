...
nb_train_samples = 2116  # Total number of train samples. NOT including augmented images
nb_validation_samples = 20  # Total number of train samples. NOT including augmented images.
...
def train(train_data_dir, validation_data_dir, model_path):
    ...
    #softmax: predict 1 of many class, sigmoid: binary and multilabel classification 
    ...
    model.compile(optimizer='adam',
                  loss='categorical_crossentropy',  # binary_crossentropy if multi-class classifier
                  metrics=['accuracy'])
    ...
    # save weights of best training epoch: monitor either val_loss or val_acc
    final_weights_path = os.path.join(os.path.abspath(model_path), 'model_finetune.h5')
    callbacks_list = [
    ...
    with open(os.path.join(os.path.abspath(model_path), 'model_finetune.json'), 'w') as json_file:
...
if __name__ == '__main__':
    if not len(sys.argv) == 3:
        ...
        validation_dir = os.path.join(os.path.abspath(data_dir), 'val_dir')  # each class should have it's own folder
        model_dir = os.path.abspath(sys.argv[2])

        if not os.path.exists(os.path.join(os.path.abspath(data_dir), 'preview')):
