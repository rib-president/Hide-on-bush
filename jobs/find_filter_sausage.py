...
def chase_point(master, stat):
    ...
    for most in most_long:
        most = ast.literal_eval(most)
        filter_list.append(most)
        
    if len(long_too) == len(most_long):
        for too in long_too:
            too = ast.literal_eval(too)
            filter_list.append(too)
    
    
    with open('inverse_filter_confused_5product_new.csv', 'a') as inf:
