...
def main(arguments):
    ...
    for image in images:
        ...
        probability = np.sum(predictions, axis=0)
        overall_result = np.argmax(probability)
        max_probability = round(np.max(probability)*10, 2)
        ...
        print(image_path)
        print(overall_result, label_map[overall_result], max_probability)
